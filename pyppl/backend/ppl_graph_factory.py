...
class GraphFactory(object):
    ...
    def create_observe_node(self, dist: AstNode, value: AstNode, parents: set):
        ...
        result = Vertex(name, ancestors=parents, dist_code=d_code, distribution_name=_get_dist_name(dist),
                        observation=v_code, observation_value=obs_value)
        self.nodes.append(result)
        return result
    ...
    def create_sample_node(self, dist: AstNode, size: int, parents: set):
        ...
        result = Vertex(name, ancestors=parents, dist_code=code, distribution_name=_get_dist_name(dist),
                        sample_size=size)
        self.nodes.append(result)
        return result

    def generate_code(self, model_template: Optional[str] = None):
        code_gen = GraphCodeGenerator(self.nodes, self.code_generator.state_object)
        return code_gen.generate_model_code()
